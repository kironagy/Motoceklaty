...
const MAX_ACTIVE_CUSTOMERS = Math.max(1, Number(process.env.MAX_ACTIVE_CUSTOMERS || 5));
const CUSTOMER_IDLE_MS = Math.max(10000, Number(process.env.CUSTOMER_IDLE_MS || 60000));
...
function queuePosition(key) {
    return waitingQueue.findIndex(item => item.key === key);
}
...
async function closeCustomerPage(key, reason = 'idle') {
    ...
    delete pages[key];
    activeCustomers.delete(key);
    processingCustomers.delete(key);

    console.log(`🧹 Closed customer tab key=${key} reason=${reason} active=${activeCustomers.size}/${MAX_ACTIVE_CUSTOMERS}`);
...
function scheduleIdleClose(key) {
    ...
    idleTimers[key] = setTimeout(async () => {
        if (processingCustomers.has(key)) {
            scheduleIdleClose(key);
            return;
        }

        const last = customerLastActivity[key] || 0;
        const idleFor = Date.now() - last;

        if (idleFor >= CUSTOMER_IDLE_MS) {
            await closeCustomerPage(key, `idle_${CUSTOMER_IDLE_MS}ms`);
            return;
        }
...
function drainWaitingQueue() {
    while (activeCustomers.size < MAX_ACTIVE_CUSTOMERS && waitingQueue.length > 0) {
        const item = waitingQueue.shift();

        if (!item || activeCustomers.has(item.key)) {
            if (item?.resolve) item.resolve();
            continue;
        }

        activeCustomers.add(item.key);
        customerLastActivity[item.key] = Date.now();
        clearIdleTimer(item.key);

        console.log(`▶️ Activated waiting customer key=${item.key} active=${activeCustomers.size}/${MAX_ACTIVE_CUSTOMERS} waiting=${waitingQueue.length}`);

        item.resolve();
    }
}

async function acquireCustomerSlot(key) {
    key = normalizeKey(key);
    customerLastActivity[key] = Date.now();

    if (activeCustomers.has(key)) {
        clearIdleTimer(key);
        return;
    }

    if (activeCustomers.size < MAX_ACTIVE_CUSTOMERS) {
        activeCustomers.add(key);
        clearIdleTimer(key);
        console.log(`✅ Activated customer key=${key} active=${activeCustomers.size}/${MAX_ACTIVE_CUSTOMERS}`);
        return;
    }

    console.log(`⏳ Customer queued key=${key} position=${waitingQueue.length + 1} active=${activeCustomers.size}/${MAX_ACTIVE_CUSTOMERS}`);
...
'--use-gl=swiftshader',
    ...
    console.log(`⚙️ MAX_ACTIVE_CUSTOMERS=${MAX_ACTIVE_CUSTOMERS} CUSTOMER_IDLE_MS=${CUSTOMER_IDLE_MS}`);
}
...
async function fillComposer(page, message) {
    ...
    await composer.click({ timeout: 30000, force: true });
    await sleep(500);
    ...
    try {
        await composer.fill(text, { timeout: 30000 });
    } catch (e) {
...
async function uploadMediaFiles(page, mediaPaths = []) {
    if (!mediaPaths.length) return 0;

    const missingPaths = mediaPaths.filter(p => !fs.existsSync(p));

    if (missingPaths.length) {
        throw new Error('media file missing: ' + missingPaths.join(', '));
    }

    console.log(`📎 Uploading ${mediaPaths.length} media file(s) to ChatGPT`);
    ...
    console.log(`✅ Media uploaded count=${mediaPaths.length}`);
...
async function sendMessage(page, message, mediaPaths = []) {
    console.log('TRACE SEND 01 getComposer start');

    await getComposer(page);

    console.log('TRACE SEND 02 getComposer done');

    if (mediaPaths.length) {
        console.log('TRACE SEND 03 uploadMediaFiles start', {
            count: mediaPaths.length,
            paths: mediaPaths,
        });

        const uploadedCount = await uploadMediaFiles(page, mediaPaths);

        console.log('TRACE SEND 04 uploadMediaFiles done', {
            uploadedCount,
            expected: mediaPaths.length,
        });

        if (uploadedCount !== mediaPaths.length) {
            throw new Error(`media upload failed: uploaded ${uploadedCount}/${mediaPaths.length}`);
        }

        console.log('TRACE SEND 05 wait media processing start');
        await sleep(15000);
        console.log('TRACE SEND 06 wait media processing done');
    }

    console.log('TRACE SEND 07 fillComposer start', {
        message_length: String(message || '').length,
        message_start: String(message || '').slice(0, 200),
    });

    await fillComposer(page, message);

    console.log('TRACE SEND 08 fillComposer done');

    await sleep(1500);

    console.log('TRACE SEND 09 clickSend start');

    await clickSend(page);

    console.log('TRACE SEND 10 clickSend done');
}
...
async function askChatGPT(message, conversationKey, memoryPrompt = '', mediaPaths = []) {
    ...
    return enqueuePage(key, async () => {
        ...
        console.log('TRACE ASK 01 start', {
            key,
            phone,
        ...
        console.log('TRACE ASK 02 acquireCustomerSlot start');
        await acquireCustomerSlot(key);
        console.log('TRACE ASK 03 acquireCustomerSlot done');

        processingCustomers.add(key);
        clearIdleTimer(key);
        customerLastActivity[key] = Date.now();

        try {
            console.log('TRACE ASK 04 getPageForCustomer start');
            const page = await getPageForCustomer(key);
            console.log('TRACE ASK 05 getPageForCustomer done', {
                url: page.url(),
            });

            console.log('TRACE ASK 06 getComposer before message start');
            await getComposer(page);
            console.log('TRACE ASK 07 getComposer before message done');
            ...
            console.log('TRACE ASK 08 buildMessage done', {
                isFirstMessage,
                final_message_length: String(finalMessage || '').length,
            });

            console.log('TRACE ASK 09 getAssistantMessages before start');
            const before = await getAssistantMessages(page);
            const beforeCount = before.length;
            const beforeLast = before[before.length - 1] || '';
            console.log('TRACE ASK 10 getAssistantMessages before done', {
                beforeCount,
                beforeLastStart: String(beforeLast || '').slice(0, 120),
            });
            ...
            console.log('TRACE ASK 11 sendMessage start');
            await sendMessage(page, finalMessage, mediaPaths);
            console.log('TRACE ASK 12 sendMessage done');
            ...
            console.log('TRACE ASK 13 saveStore before wait reply');
            saveStore();
            console.log('TRACE ASK 14 saveStore done');

            const replyTimeout = mediaPaths.length ? 600000 : (isFirstMessage ? 600000 : 420000);

            console.log('TRACE ASK 15 waitForAssistantReply start', {
                replyTimeout,
                beforeCount,
            });
            ...
            console.log('TRACE ASK 16 waitForAssistantReply done', {
                reply_length: String(reply || '').length,
                reply_start: String(reply || '').slice(0, 200),
            });
            ...
            console.log('TRACE ASK 17 saveStore after reply start');
            saveStore();
            console.log('TRACE ASK 18 saveStore after reply done');

            return reply;
        } catch (error) {
            console.error('TRACE ASK ERROR', {
                key,
                message: error?.message || String(error),
                stack: error?.stack,
            });

            await closeCustomerPage(key, 'error');
            throw error;
        } finally {
            processingCustomers.delete(key);
            customerLastActivity[key] = Date.now();
            ...
            console.log('TRACE ASK 19 finally done', {
                key,
                active_customers: activeCustomers.size,
waiting_customers: waitingQueue.length,            });
        }
    });
}
...
app.get('/health', (req, res) => {
    res.json({
        ...
        opened_pages: Object.keys(pages).filter(key => pages[key] && !pages[key].isClosed()).length,
        max_active_customers: MAX_ACTIVE_CUSTOMERS,
        active_customers: activeCustomers.size,
        waiting_customers: waitingQueue.length,
        active_keys: Array.from(activeCustomers),
        waiting_keys: waitingQueue.map(item => item.key),
        idle_timeout_ms: CUSTOMER_IDLE_MS,
    });
});
