...
async function uploadMediaFiles(page, mediaPaths = []) {
    ...
    while (Date.now() - startedAt < timeoutMs) {
        ...
        const stillUploading =
            /uploading|جار.*تحميل|جاري.*تحميل|upload/i.test(pageText);

        const hasComposer = await findComposer(page).catch(() => null);

        if (hasComposer && !stillUploading) {
    ...
    await page.screenshot({
        path: `media-upload-timeout-${Date.now()}.png`,
        fullPage: true,
    }).catch(() => {});

    throw new Error(`media upload timeout: ${mediaPaths.length} file(s)`);
}
