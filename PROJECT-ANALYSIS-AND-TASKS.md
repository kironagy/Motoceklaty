...
| B11 | `ApplicationHandler` unresolvable (namespace/file mismatch) | ✅ Done | **Found live in production logs, not by static reading** — the entire "apply for installment" flow was silently failing (customer got zero reply). Fixed, verified, worker restarted with the fix. |
| B12 | Filament memory edits invisible to AI for up to 5 min | ✅ Done | **Reported by the user directly.** Dead cache-invalidation hook (referenced a class that never existed) wired to the real caches. Verified live with `touch()` + cache checks. |
| §3 #2 Full memory dump | AI token usage | ✅ Done | Relevance-filtered via existing (previously unused) `AiMemoryResolver::relevantMemories()`. Gemini caching investigated and confirmed unavailable on this account (see §3 Implementation Notes) before choosing this approach. |
| §3 #3 OCR payload duplication | AI token usage | ✅ Done | Only the most recent document-bearing message keeps full OCR text; older ones summarized. |
| §3 #5 Token estimation | AI token usage | ✅ Done | Was 1 char = 1 token; corrected to `/4` based on a real measured Gemini response. |
| §3 #6 No truncation caps | AI token usage | ✅ Done | Defensive `mb_substr` caps added; no effect in normal use. |
| §3 #1 Duplicate AI calls, #4 no fast-path | AI token usage | ⬜ Deliberately not done | Both would change decision logic/behavior, not just efficiency — held pending an explicit go-ahead, per your "don't change the final logic" instruction. |
| §5 / T1.3 | Google Cloud Vision OCR | ⏸ Paused — stopped on request | An `OcrProviderInterface` + `PaddleOcrClient implements` no-op scaffold exists (zero behavior change); no further OCR work done or planned until you say so. |
...
| T1.1 | Bugs and Reliability Fixes | ✅ Done (all listed bugs) / 🟡 tests still open | B2/B3/B4/B6/B7/B11 all fixed and verified live; alerting hooks (§7) and an automated test suite for these fixes are the only remaining pieces of this task group. |
| T1.2 | AI Token Usage and Refactoring | 🟡 Partial | #2/#3/#5/#6 done and verified; #1/#4 deliberately held (behavior-risk, needs your go-ahead); token/cost metrics (§7) still open. |
| T1.3 | Google Cloud Vision OCR | ⏸ Paused | Stopped on request after adding the harmless interface scaffold. |
...
### B11 — CRITICAL — `App\Services\Whatsapp\Handlers\ApplicationHandler` did not exist as an autoloadable class (the entire "apply for installment" flow was silently broken)
**Status: ✅ Fixed and verified live.**
- **File**: was [app/Services/Handlers/ApplicationHandler.php](app/Services/Handlers/ApplicationHandler.php); imported from [WhatsappIntentRouter.php:13](app/Services/WhatsappIntentRouter.php:13).
- **How found**: not from static reading — caught live in `storage/logs/laravel.log`, real timestamps `19:35`-`19:42` on the day this was found: `WhatsappIntentRouter simple error {"message":"Target class [App\\Services\\Whatsapp\\Handlers\\ApplicationHandler] does not exist."}`. Reproduced on demand via `app(\App\Services\Whatsapp\Handlers\ApplicationHandler::class)` in tinker before the fix.
- **Exact problem**: the class declared `namespace App\Services\Whatsapp\Handlers;` but the file lived at `app/Services/Handlers/ApplicationHandler.php` — one directory level short of what PSR-4 (`"App\\": "app/"` in `composer.json`) requires for that namespace. `WhatsappIntentRouter` imports and calls `app(ApplicationHandler::class)` when intent resolves to `application` ([WhatsappIntentRouter.php:179](app/Services/WhatsappIntentRouter.php:179), pre-B5-cleanup line number).
- **Why it happened**: unclear — most likely the file was moved into `Handlers/` without updating its `namespace` line, or vice versa. There is no classmap fallback in this project's `composer.json` (plain PSR-4 only), and the local autoloader was not in optimized/classmap mode, so the mismatch surfaced immediately rather than being masked.
- **User impact**: **any customer message that resolved to the `application` intent (i.e., trying to submit an installment application) hit an uncaught `BindingResolutionException`**, caught by `WhatsappIntentRouter`'s outer `catch (\Throwable $e)`, which returns `reply: null`. Downstream, `ProcessWhatsappMessageJobs::sendWhatsappResult()` only sends a message when `trim((string) $result['reply']) !== ''` — with `reply: null` that's always false, so **the customer received absolutely nothing**, not even an error message. This was live and reproducible at the time it was found, not a historical/fixed issue.
- **Fix applied**: changed the namespace declaration to `App\Services\Handlers` (matching the file's real location) and updated the one `use` import in `WhatsappIntentRouter.php` to match. No other file referenced the old namespace (`grep -rl` confirmed). Verified live: `app(\App\Services\Handlers\ApplicationHandler::class)` now resolves; the running `whatsapp:process-jobs` worker was restarted to pick up the fix.
- **Validation method**: `php artisan tinker --execute="app(\App\Services\Handlers\ApplicationHandler::class);"` should not throw. For an end-to-end check, drive a real conversation through the `application` intent (customer says "عايز اقدم") and confirm a reply is actually sent, not silence — and check `storage/logs/laravel.log` for any renewed `Target class ... does not exist` entries after a deploy.

### B12 — HIGH — Editing an `ai_memories` row in Filament had no effect on the AI until a cache expired on its own
**Status: ✅ Fixed and verified live.**
- **File**: [app/Models/AiMemory.php](app/Models/AiMemory.php).
- **How found**: reported by the user directly — edited a memory at `/admin/ai-memories/31/edit`, sent a WhatsApp test message, the AI's reply didn't reflect the change.
- **Exact problem**: two independent caches sit in front of the `ai_memories` table — `AiMemoryContextBuilder`'s `ai_full_memory_context` (5 min TTL) and `AiMemoryResolver`'s `active_ai_memories_for_laravel` (1 min TTL, this is the one the relevance-filtering fix from T1.2 reads from). `AiMemory::booted()` already had a `saved`/`deleted` hook whose entire purpose was to invalidate stale caches on edit — but it called `App\Services\Ai\MemoryToonIndexBuilder::class`, a class that was **never created** in this codebase. The `class_exists()` guard around it made the hook a silent no-op on every single save, so nothing ever actually got invalidated by an edit — only the caches' own TTL ever cleared them.
- **User impact**: an admin's edit in Filament could take up to 5 minutes to reach the AI, and worse, felt inconsistent/random depending on which of the two caches happened to be warm at the time — exactly matching "the change I made doesn't get sent."
- **Fix applied**: replaced the dead `MemoryToonIndexBuilder::rebuild()` calls with `AiMemoryContextBuilder::clearCache()` + `Cache::forget('active_ai_memories_for_laravel')`, called on both `saved` and `deleted`. Verified live: warmed both caches, called `AiMemory::find(31)->touch()` (simulates a Filament save), confirmed both caches were empty immediately after — no waiting, no partial staleness.
- **Validation method**: edit any memory in Filament, then within the same minute send a WhatsApp test message whose topic matches that memory — the change should be visible in the very next reply. Programmatically: `Cache::has('ai_full_memory_context')` and `Cache::has('active_ai_memories_for_laravel')` should both be `false` immediately after any `AiMemory` save/delete.
...
|---|---|---|
| #1 Duplicate AI calls per message (classify + fallback, classify + application extraction) | [WhatsappIntentRouter.php:37](app/Services/WhatsappIntentRouter.php:37), [:254](app/Services/WhatsappIntentRouter.php:254), [ApplicationHandler.php:32](app/Services/Handlers/ApplicationHandler.php:32) | ⬜ Confirmed, **not fixed** — deliberately left alone (see Implementation Notes: highest behavior-risk item, holding pending explicit go-ahead) |
| #2 Full unfiltered memory dump on every fallback reply | [AiMemoryContextBuilder.php](app/Services/AiMemoryContextBuilder.php) | ✅ **Fixed** — now relevance-filtered (top 18 by keyword match, falls back to full dump if fewer than 5 match). ~35-45% smaller on real test messages, verified live. |
| #3 OCR text repeated in up to 20 subsequent classify() prompts | [AiIntentClassifier.php](app/Services/AiIntentClassifier.php) | ✅ **Fixed** — full OCR/media payload kept only for the single most recent document-bearing message in the 20-message window; older ones collapsed to a short summary. |
| #4 No fast-path before AI classify for trivial messages | [AiIntentClassifier.php:11-59](app/Services/AiIntentClassifier.php:11) | ⬜ Confirmed, **not fixed** — see Implementation Notes. |
| #5 Token estimate = `mb_strlen($prompt)`, not real tokenization | [GeminiClient.php](app/Services/GeminiClient.php) | ✅ **Fixed** — was billed as 1 char = 1 token; a live measured test (real `usageMetadata.promptTokenCount` on Arabic content) showed that was ~3.7x too pessimistic. Changed to `mb_strlen($prompt) / 4`. |
| #6 No truncation cap on message/payload/memory injected into prompts | `AiPromptBuilder.php`, `AiIntentClassifier.php` | ✅ **Fixed** — defensive `mb_substr` caps added (message 4000 chars, memory context 20000 chars, each historical message body 1000 chars). Only ever triggers on abnormal input, not in normal use. |
...
- **#10 `AiMemoryContextBuilder` cache key is global, not per-tenant/bot.** `Cache::remember('ai_full_memory_context', ...)` ([AiMemoryContextBuilder.php:25](app/Services/AiMemoryContextBuilder.php:25)) is a single cache key shared across all bots/conversations. Not a token-cost bug by itself (it's the same memory content for everyone today), but it means there is no way to scope memories per bot without a code change, and it's worth flagging alongside the "no filtering" issue (#2) since both point at the same root cause: memory context is all-or-nothing. **Status: ⬜ not addressed** — orthogonal to the relevance-filtering fix for #2, still open.

### Implementation notes: what was actually built for #2/#3/#5/#6

**Gemini caching was investigated and ruled out first.** Before writing any filtering logic, tested whether Gemini's own caching could solve this "for free" (same full content, lower bill, zero intelligence tradeoff) — the ideal outcome. Two real API calls were made with the account's actual key:
1. `POST /v1beta/cachedContents` (explicit context caching) → `429 RESOURCE_EXHAUSTED`, `"TotalCachedContentStorageTokensPerModelFreeTier limit=0"` — **explicit caching is not available at all on this account's free tier**, regardless of content size.
2. Sent the exact same ~4800-token prompt twice in a row to `gemini-3.1-flash-lite` and compared `usageMetadata` — no `cachedContentTokenCount` appeared either time, both calls billed the full prompt. **Implicit/automatic caching is not happening either** on this tier/model.

Conclusion: on the current (free-tier) setup, tokens can only be reduced by sending less, not by caching. This directly shaped the fix below.

**#2 (memory dump) fix**: `AiMemoryResolver::relevantMemories($text, $intent, $limit)` already existed in the codebase (keyword/token-overlap scoring against memory titles+content, used elsewhere only for the deterministic template-reply lookup) but was never wired into the AI-prompt path. `AiMemoryContextBuilder::buildForMessage()` now calls it with the current message + last 3 conversation messages as the scoring text, `limit=18`. If fewer than 5 memories score above zero, the message doesn't have enough topical signal to trust filtering, so it falls back to the full 40-memory dump — meaning this change can only make a call *cheaper*, never *less informed*, than the pre-fix baseline. Measured live: 13825 chars (full) → 7000-8700 chars on 4 representative test messages (~35-45% reduction), growing more favorable as more memories get added over time (the count sent is now capped at 18 regardless of how many total memories exist).

**#3 (OCR duplication) fix**: added `AiIntentClassifier::recentMessagesForPrompt()` — builds the same 20-message window as before, but only the single most recent message carrying `ocr_results`/`saved_media_items` keeps its full payload; older ones are collapsed to `{media_count, media_types, ocr_status, ocr_documents_processed}`. The already-extracted structured fields (name, national ID, etc.) live in `context_payload`/`application` state separately and are unaffected — this only removes *raw re-transmission* of old document text, which the model doesn't need a second (or twentieth) time.

**Not changed, on purpose**: #1 (merging the classify()/fallback-reply calls) and #4 (a fast-path before AI classify) were both left untouched. Both would touch the actual decision logic (what the model is asked to do, or whether it's asked at all for a given message), which is exactly the "final logic" the user asked not to change. They remain open in T1.2 pending an explicit decision to take on that behavior risk.
