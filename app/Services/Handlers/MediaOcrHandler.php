...
class MediaOcrHandler
{
    public function handle(
        ...
        /*
         * مفيش ولا ملف اتقرا. الرسالة القديمة كانت "تمام يا فندم، استلمت
         * الملفات، هتتحول للمراجعة" - وده تأكيد كاذب: الملف يمكن يكون
         * مش موجود على الديسك أصلًا أو نوعه مش مدعوم، والعميل بيقعد
         * يستنى رد على حاجة معدّتش. بنقول الحقيقة ونطلب إعادة الإرسال.
         */
        $unsupported = collect($results)
            ->where('ok', false)
            ->pluck('error')
        ...
        return $this->reply(
            $conversation,
            $unsupported
                ? 'معلش يا فندم، نوع الملف ده مش بقدر أفتحه. ابعتهولي صورة (JPG أو PNG) أو PDF لو سمحت.'
                : 'معلش يا فندم، الملف مقدرتش أفتحه. ممكن تبعته تاني؟',
            $status,
            true
        );
    }
