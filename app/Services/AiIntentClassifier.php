...
- **التضييق ده شرطه إن الكلمة تكون فعلاً بداية اسم واحد من المعروضين، مش
  مجرد إنها تشبهه.** الاسم اللي فيه حروف زيادة في آخره اسم **تاني خالص**:
  "دايونج" مش تضييق على "دايو 2"، و"بلسر" مش تضييق على "بوكسر". في
  الحالة دي target = new_machine وmachine_query = الاسم اللي العميل كتبه
  بالحرف. لو مش متأكد إن الكلمة واحدة من المعروضين، اعتبرها موديل جديد -
  البحث هيتصرف، لكن التضييق الغلط بيرجّع للعميل موديل تاني خالص.
...
- recommendation: العميل مش عارف يختار وعايزنا نختارله، أو حاطط ميزانية وعايز يعرف المتاح فيها، أو بيفاصل على سعر. أمثلة: "رشحلي حاجة كويسة"، "إيه الأنسب لشغل التوصيل؟"، "أي حاجة كويسة"، "إنت اختارلي"، "مش عارف أختار"، "معايا ٣٠ ألف، فيه إيه؟"، "عايز حاجة في حدود ٤٠ ألف"، "ينفع آخدها بـ ٢٠ ألف؟"، "مفيش خصم؟". لو فيه رقم فلوس في الرسالة حطه في budget. دي **مش** installment_calc: العميل مسمّاش موديل أصلاً، فمفيش حاجة تتحسب. وممنوع ترجّع installment_calc لرسالة فيها ميزانية من غير اسم موديل.
...
- **ممنوع تقصّ اسم الموديل أو تصلّحه.** machine_query لازم يبقى الاسم زي
  ما العميل كتبه بالحرف. "دايونج" تفضل "دايونج" - ممنوع تخليها "دايو"
  حتى لو شكلها غلطة إملائية أو شبه اسم تعرفه. البحث عنده قاموس بيتعامل
  مع الأخطاء الإملائية، وقص الاسم بيحوّل السؤال لموديل تاني موجود فعلاً
  بسعر مختلف - وده أسوأ من إن البحث ميلاقيش حاجة.
...
PROMPT
    ...
    private function fallback(): array
    {
        return [
            ...
            'deposit_mentioned' => false,
            /*
             * الميزانية اللي العميل قالها لما بيطلب ترشيح ("معايا ٣٠
             * ألف، فيه إيه؟"). مختلفة عن deposit - دي سقف سعر المكنة،
             * مش مقدم على قسط.
             */
            'budget' => null,
            'system' => null,
    ...
    private function normalizePlanFields(array $plan): array
    {
        ...
        $plan['budget'] = isset($plan['budget']) && $plan['budget'] !== null
            ? (float) $plan['budget']
            : null;
...
قواعد مهمة:
- stated_age: لو العميل قال سنه بالكلام ("سني ٢٨"، "عندي ٣٥ سنة"، "أنا ٤٠")،
  حط الرقم في stated_age. ده رقم بيتقارن بعدين بالسن المستخرج من الرقم
  القومي عشان نكشف رقم مكتوب غلط. لو مقالش سنه سيبه null، وممنوع تحسبه
  من الرقم القومي - إحنا عايزين اللي هو قاله بلسانه بس.
- deposit: لو العميل قال مقدم ("معايا مقدم ٥ الاف"، "هدفع ١٠ الاف مقدم")،
  حط الرقم في deposit. كان بيضيع خالص قبل كده والعميل بيضطر يعيده.
...
- نفس الفكرة لـ national_id: لو الرسالة رقم بس (10 لـ14 خانة) ومش شكل رقم
  ...
  اعتبرها phone.
- **مهم جدًا لـ national_id**: حتى لو الرسالة مش رقم بس (فيها كلام مع الرقم)،
  لو الرسالة تحتوي على تسلسل أرقام طوله 14 خانة ومش بادئ بـ01، استخرجه كـ
  national_id. ده بيحصل كتير لما العميل يقول "ده الرقم القومي اهو 30410012208373"
  أو "مانا بعتلك الرقم القومي اهو" وهو باعت الرقم في رسالة قبلها (ده مش
  national_id في الحالة دي) أو لما يبعت "30410012208373 ده رقمي". الفحص
  بالبحث في نص الرسالة عن أي sequence من 14 رقم متتالي مش مسبوق بـ01.
...
- في المدن الجديدة (٦ أكتوبر، الشيخ زايد، التجمع، الرحاب) العنوان بيتكتب
  بصيغة "الحي/المربع/المجاورة + رقم الفيلا" من غير اسم شارع خالص، مثل
  "اكتوبر ١٥ أ - مربع ٣ - فيلا ١١٥". دي صيغة عنوان سليمة تمامًا - ممنوع
  تعتبرها كلام مش عنوان، وممنوع تحذف منها "مربع ٣" أو "فيلا ١١٥".
- لما العميل يرد على سؤال عن مكوّن ناقص في العنوان برد قصير (زي "فيلا ١١٥"
  أو "عمارة ١٢ إيجار")، رجّع العنوان **المدمّج**: اللي كان محفوظ في
  known_context.current_application + الجزء الجديد، مش الجزء الجديد لوحده.
...
- صيغ زي "انا شغال على المكنة"، "هشتغل عليها"، "شغال بيها"، "المكنة دي لشغلي"، "شغال طلبات"، "شغال دليفري"، "شغال اوبر/اندرايف/كريم"، "سواق تطبيقات" كلها إجابة على سؤال طبيعة الشغل: job_type = "مندوب توصيل/سواق تطبيقات" وincome_proof = "لا يوجد". دي **إجابة على سؤال طبيعة الشغل**، مش رسالة فاضية - ممنوع تسيب job_type = null فيها. الصيغ دي **مبتحددش نوع المركبة** - "شغال طلبات" لوحدها معناها دليفري بس، مش موتوسيكل ولا أي مركبة تانية؛ نوع المركبة بيتحدد بس بالقاعدة الخاصة بـ work_vehicle تحت.
