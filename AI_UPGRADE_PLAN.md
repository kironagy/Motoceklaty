...
- [x] محامي بيتوقف من أول رسالة بعد ما الوظيفة تتعرف — **متأكد منه end-to-end** (بعد تصليح باگ ضياع إجابة «كاش ولا تقسيط؟» — قبل كده المسار الكامل كان بيقف قبل ما يوصل للوظيفة أصلًا؛ شوف «مراجعة خارجية» بند ١، والتوحيد العربي في بند ٢)
...
**Status:** `IN_PROGRESS` (2026-08-25) — 4.1 اتعمل بنسخة مختصرة (١٦ حالة مش ٤٠-٦٠)، **١٦/١٦ ناجحة**. 4.2 و4.3 اتأجّلوا
...
- **١٦ حالة** (كانت ٨) — الأولانية ٨ كل واحدة فيهم سلوك اتصلّح واتأكد منه فعليًا في Phase 1/2/3، والتانية ٨ بتغطي السلوك اللي حاميه دلوقتي شروط الـ regex في الراوتر (تحويل لموظف، شكوى، مصاريف إدارية، حساب قسط صريح، موديلات ماركة، توصيل، فروع، حالة الطلب) — عشان مهمة 2.5 يبقى قدامها شبكة أمان تفشل عندها. الأصل: الطلب المركّب (سعر+صور)، الطلب المركّب من التقرير الأصلي (موديل+مدة+مقدم+تفاصيل)، تضييق «اصلي»، عدم كسر استعلام العائلة العادي، بوابة المهنة الممنوعة، عدم رفض مهنة سليمة، تحية بسيطة، سؤال نظام تقسيط.
...
**تحديث المراجعة (2026-08-25):** الأداة اتوسّعت لـ **١٦ حالة** واتضاف ليها:
- `--filter=` لتشغيل حالات بعينها بالاسم.
- `--delay=` (افتراضي ٣ ثواني) بين كل دور وكل حالة — الموديل الأقوى سقفه **10 rpm للمفتاح**، والتشغيل من غير مهلة كان بيولّد فشل مؤقت يتفسّر غلط على إنه انحدار.

**Verify (اتنفذ فعليًا):**
```bash
php artisan ai:golden-set --delay=4
```
النتيجة: **١٦ من ١٦ ناجحة**.

> ⚠️ **تصحيح لتشخيص سابق:** الفشل في حالة «بوابة المهنة الممنوعة» **مكانش تذبذب بيئة** زي ما اتكتب هنا قبل كده — كان **باگ حقيقي** اتلاقى بتتبع اللوج (تفاصيله في «مراجعة خارجية» تحت): إجابة العميل على سؤال «كاش ولا تقسيط؟» كانت بتضيع، فالتقديم يفضل يسأل نفس السؤال ومايوصلش لـ `job_type` أصلًا. اتصلّح، والحالة بقت بتعدي بثبات.
...
**ملاحظة صريحة:** ١٦ حالة، لسه مش الـ ٤٠-٦٠ اللي الخطة طلبتها. ضيف حالة جديدة هنا كل مرة تلاقي/تصلّح باگ حقيقي - بالظبط زي ما القاعدة في أول الملف بتقول.
...
# 🔍 مراجعة خارجية للمراحل ١–٤ (2026-08-25)

مراجعة للـ ٤ كوميتات اللي نفّذت Phase 1→4. **التقييم العام: التنفيذ سليم ومتحفظ في محله** — الانحرافات عن الخطة الأصلية (steps بنفس شكل الحقول بدل schema جديد، tool dispatcher خفيف بدل function-calling، تأجيل 2.4/2.5) كلها مبرّرة ومكتوبة بصراحة. اللي اتلاقى غلط واتصلّح في المراجعة دي:

### 1. باگ حقيقي: إجابة «كاش ولا تقسيط؟» كانت بتضيع → التقديم في loop
`WhatsappIntentRouter` بيعتبر أي `installment_calc` بثقة ≥ 0.5 وسط تقديم شغال «سؤال مقاطع» ويروح لحساب القسط. لكن كلمة **«تقسيط» لوحدها بعد سؤال «حضرتك عاوز تدفع كاش ولا تقسيط؟» هي إجابة السؤال، مش سؤال جديد**. النتيجة المرصودة في `ai:golden-set`: `missing_fields=[payment_method]` على ٦ أدوار متتالية، الاسم والرقم القومي والموبايل والوظيفة كلهم بيتقالوا والبوت بيعيد نفس السؤال — و`job_type` عمره ما بيتفحص، يعني **بوابة المهن الممنوعة (مهمة 3.2) عمرها ما اشتغلت في المسار الكامل**.

**التصليح:** لو التقديم مستني `payment_method` والرسالة قصيرة (≤ ٣ كلمات) وفيها كاش/تقسيط/قسط → ترجع للتقديم دايمًا، مش للـ installment_calc. (مفيش دالة `isSomething()` جديدة — الشرط inline، احترامًا لقاعدة رقم ٤).

> ده كمان **يصحّح تشخيص مكتوب في مهمة 4.1**: الحالة الفاشلة اتفسّرت وقتها على إنها تذبذب بيئة/مفاتيح. مكانتش.

### 2. `bannedProfessionReason()` كانت بتفوت كتابات شائعة
الفحص كان `str_contains` على نص خام: `محامي` بس (مش `محامى`/`محاماة`)، `قاضي` بس (مش `قاضى`)، و`امين شرطه`/`أمين شرطة` بالحرف — يعني «أمين الشرطة» كانت بتعدي. اتعمل توحيد عربي (`أإآ`→`ا`، `ة`→`ه`، `ى`→`ي`، وشيل «ال») قبل المطابقة، والمهن الممنوعة بقت تتفحص على النص الموحّد + سياق «الداخلية» لـ أمين/معاون (عشان «أمين مخزن» و«معاون مدير مبيعات» يفضلوا سليمين). متأكد بـ ١٨ حالة.

### 3. الـ planner كان بيقع على `fallback()` بصمت لو الرد اتقطع
بعد ما البرومبت كبر (الميموري كلها اتضافت له في مهمة 1.7) وبقى على موديل بيصرف جزء من الميزانية على تفكير داخلي، أي قص في الـ JSON = `intent=unknown` للرسالة كلها. اتضافت **إعادة محاولة واحدة بـ `thinkingBudget = 0`** في `classify()` و`extractApplicationData()` — نداء زيادة وقت الفشل بس.

### 4. حاجات صغيرة
- الـ docblock بتاع `handle()` اتمسح بالغلط في Phase 2 (اتحط مكانه docblock الخاصية الجديدة) — اترجع.
- `steps[]` متداخلة جوه step اتشالت في `normalizePlan()` (كانت بتتحمل من غير ما تتنفذ).

### 5. ⚠️ اكتشاف تشغيلي مهم: ترقية الموديل (مهمة 1.4) شبه معطّلة فعليًا
في تشغيلة `ai:golden-set` كاملة، **كل** نداء طلع معاه:
```
Gemini preferred model unavailable, downgrading {"from":"gemini-3.7-flash","to":"gemini-3.1-flash-lite"}
```
السبب: `gemini-3.7-flash` سقفه **250 نداء/يوم للمفتاح**، والمفتاحين الشغالين (7, 8) خلّصوا الـ 250 وعندهم `cooldown_until` لآخر اليوم. المفتاحين التانيين (9, 10) `is_active=false`. يعني عمليًا: **500 نداء/يوم للموديل القوي، وبعدها كل حاجة بترجع للموديل الضعيف** — نفس الموديل اللي المرحلة الأولى كلها كانت بتحاول تبعد عنه.

- النزول التلقائي شغال صح (الرد مبيموتش) — دي كانت مهمة 1.8-ب وهي سليمة.
- بس الجودة اللي المراحل دي اتبنت عليها **مش هي الجودة اللي العميل بياخدها أغلب اليوم**.
- **أولوية أولى قبل أي مهمة جديدة:** جدّد مفتاحي Gemini المعطّلين (9, 10)، أو ضيف مفاتيح جديدة. أي تقييم لجودة الرد قبل كده بيقيس الموديل الضعيف.

### 6. ملاحظات مرصودة، متصلّحتش عن قصد
- **رسايل خارجة مزدوجة مع الـ extra steps:** كل handler بينده `saveOutgoing()` بنفسه، فالـ step الإضافي بيسجّل رسالة خارجة تانية في الداتابيز رغم إن العميل بياخد رسالة واحدة مدموجة. مش مؤثر على العميل (الـ controller بيبعت `$result['reply']` بس)، بس تاريخ المحادثة في الأدمن بيبان فيه سطرين. تصليحه = تحريك `saveOutgoing` لمستوى `handle()`، وده يستاهل مهمة لوحدها.
- **`$conversation->from` مش موجود:** الأعمدة الحقيقية `phone` / `real_phone`، فـ `'from' => $conversation->from ?? null` اللي بيتبعت لـ `AiComplexReplyService` **دايمًا null**. باگ قديم مش من الكوميتات دي، بس يستاهل يتصلّح.
- **مهمة 2.5 (مسح شروط الـ regex):** بعد قراية أماكن الاستخدام، **مش كلها overrides**. `detectIntent()` في السطور 449/473/521 بتشتغل **لما الـ AI يفشل أو يرجّع unknown** — دي شبكة أمان، ومع اكتشاف رقم ٥ فوق (النزول للموديل الضعيف يوميًا) قيمتها أعلى مش أقل. التوصية المعدّلة: امسح الـ overrides اللي بتتقدّم على خطة صالحة (زي شرط 485)، وسيب مسارات الـ fallback.
...
**التوصية الحالية (بعد المراجعة الخارجية):**
1. **جدّد/راجع مفتاحي Gemini المعطّلين** (`id 9, 10`) - من غير كده الموديل القوي بيخلص بعد ٥٠٠ نداء وكل باقي اليوم شغال على الموديل الضعيف (تفاصيل في «مراجعة خارجية» بند ٥). ده أهم بند في الملف دلوقتي.
2. زوّد `AiGoldenSet::cases()` بحالات جديدة كل مرة تلاقي باگ.
3. ارجع لمهمة ٢.٥ **بالنطاق المعدّل** (بند ٦ في المراجعة): امسح الـ overrides اللي بتتقدّم على خطة AI صالحة، سيب مسارات الـ fallback - دالة واحدة في المرة، و`php artisan ai:golden-set --delay=4` بعد كل واحدة.
4. بعدها 2.4 مع الـ validator الرقمي.
